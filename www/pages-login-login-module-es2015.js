...
/* harmony import */ var _ionic_native_toast_ngx__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! @ionic-native/toast/ngx */ "./node_modules/@ionic-native/toast/__ivy_ngcc__/ngx/index.js");
...
let LoginPage = class LoginPage {
    constructor(translate, Toast, core, storage, alertCtrl, routerOutlet, navCtrl, platform, Facebook) {
        this.Toast = Toast;
        this.core = core;
    ...
    login() {
        if (this.loadingFacebook || this.loading || this.form.invalid)
            return;
    ...
    facebook(isTry) {
        if (this.loadingFacebook || this.loading)
            return;
        ...
        this.Facebook.login(['email']).then((res) => {
            if (res && res['authResponse']) {
                ...
                this.core.request('m_facebook/login', option).subscribe(res => {
                    ...
                    this.loadingFacebook = false;
                    let message;
                    if (this.trans['register'][err['error']['code']])
                        message = this.trans['register'][err['error']['code']];
                    else
                        message = this.trans['register']['default'];
            ...
            this.Facebook.logout().then(() => {
                if (!isTry)
                    ...
                    this.Toast.showLongCenter(this.trans['register']['default']).subscribe(() => { }, () => { });
            }).catch(() => { });
        });
    }
    register() {
    ...
    handlerForgot(data) {
        return Object(tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"])(this, void 0, void 0, function* () {
            ...
            this.core.request('m_users/password', option).subscribe(res => {
                ...
                if (res)
                    this.Toast.showLongCenter(this.trans['forgot']['success']).subscribe(() => { }, () => { });
                else
                    this.Toast.showLongCenter(this.trans['forgot']['mail']).subscribe(() => { }, () => { });
            }, err => {
                // when request error
                this.core.hideLoading();
